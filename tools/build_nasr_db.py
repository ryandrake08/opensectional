...
manual extraction. Imports CSV tables, ESRI shapefiles, AIXM 5.0
XML, and TFR XNOTAM data, adds decimal coordinates where needed,
and creates R-tree spatial indexes for bounding box queries.

Usage:
    python3 build_nasr_db.py <csv.zip> <shapefile.zip> <aixm.zip> <dof.zip> <adiz.geojson> <tfr_dir> <output.db>
"""
...
def main():
    if len(sys.argv) != 8:
        print(f"Usage: {sys.argv[0]} <csv.zip> <shapefile.zip> <aixm.zip> <dof.zip> <adiz.geojson> <tfr_dir> <output.db>")
        sys.exit(1)
    ...
    if not os.path.isdir(tfr_dir):
        print(f"Error: {tfr_dir} not found (expected directory)")
        sys.exit(1)
    ...
    tables = ["APT_BASE", "CLS_ARSP", "NAV_BASE", "NAV_RMK", "NAV_CKPT",
              ...
              "TFR_BASE", "TFR_AREA", "TFR_SHP", "TFR_SEG"]
    for table in tables:
