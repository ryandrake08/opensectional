...
namespace sdl
{
    struct surface::impl
    {
        ...
        explicit impl(const char* file_path) : handle(nullptr)
        {
            ...
            if(is_grayscale)
            {
                // Create RGBA surface with white RGB and grayscale as alpha
                rgba_surface = SDL_CreateSurface(loaded_surface->w, loaded_surface->h, SDL_PIXELFORMAT_ABGR8888);
                if(!rgba_surface)
                {
                ...
                // Convert grayscale to alpha with white RGB
            ...
            else if(loaded_surface->format != SDL_PIXELFORMAT_ABGR8888)
            {
                // Convert non-grayscale formats normally
                rgba_surface = SDL_ConvertSurface(loaded_surface, SDL_PIXELFORMAT_ABGR8888);
                SDL_DestroySurface(loaded_surface);
                if(!rgba_surface)
                {
            ...
            {
                // Already ABGR8888, use as-is
                handle = loaded_surface;
            }
