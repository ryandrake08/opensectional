...
namespace osect
{
    ...
    static void read_into(const ini_config& ini, const std::string& key, float& r, float& g, float& b, float& a)
    {
        if(!ini.exists(key))
        {
            return;
    ...
    static void apply_overrides(feature_style& s, const ini_config& ini, const std::string& prefix)
    {
    ...
    static void validate_style(const feature_style& s, const std::string& key)
    {
        ...
        if(!(s.min_zoom >= 0))
        {
            reject("min_zoom", s.min_zoom, "must be >= 0");
        }
        if(!(s.max_zoom >= 0))
        {
            reject("max_zoom", s.max_zoom, "must be >= 0");
        }
        if(!(s.r >= 0))
        {
            reject("r", s.r, "must be >= 0");
        }
        if(!(s.g >= 0))
        {
            reject("g", s.g, "must be >= 0");
        }
        if(!(s.b >= 0))
        {
            reject("b", s.b, "must be >= 0");
        }
        if(!(s.a >= 0))
        {
            reject("a", s.a, "must be >= 0");
        }
        if(!(s.line_width >= 0))
        {
            reject("line_width", s.line_width, "must be >= 0");
        }
        if(!(s.border_width >= 0))
        {
            reject("border_width", s.border_width, "must be >= 0");
        }
        if(!(s.dash_length >= 0))
        {
            reject("dash_length", s.dash_length, "must be >= 0");
        }
        if(!(s.gap_length >= 0))
        {
            reject("gap_length", s.gap_length, "must be >= 0");
        }
    }
    ...
    static feature_style resolve_default(const style_default& d)
    {
        ...
        if(d.color && d.color[0] != '\0')
        {
            ...
            {
                throw std::runtime_error(std::string("chart_style: bad default color '") + d.color + "' for key '" +
                                         d.key + "'");
            }
        }
        return s;
    ...
    {
        if(apt.airspace_class == "B")
        {
            return "airport_class_b";
        }
        if(apt.airspace_class == "C")
        {
            return "airport_class_c";
        }
        if(apt.airspace_class == "D")
        {
            return "airport_class_d";
        }
        if(apt.airspace_class == "E")
        {
            return "airport_class_e";
        }
    ...
    {
        if(apt.twr_type_code != "NON-ATCT")
        {
            return "airport_towered";
        }
    ...
    {
        return (nav_type == "NDB" || nav_type == "NDB/DME") ? "navaid_ndb" : "navaid_vor";
    }
    ...
    static const char* obstacle_key(int agl_ht)
    {
        if(agl_ht >= 1000)
        {
            return "obstacle_1000ft";
        }
        if(agl_ht >= 200)
        {
            return "obstacle_200ft";
        }
        return "obstacle_low";
    }
    ...
    static const char* airspace_key(const std::string& cls, const std::string& local_type)
    {
        ...
        if(local_type == "CLASS_E2")
        {
            return "airspace_e2";
        }
        if(local_type == "CLASS_E3")
        {
            return "airspace_e3";
        }
        if(local_type == "CLASS_E4")
        {
            return "airspace_e4";
        }
        if(local_type == "CLASS_E5")
        {
            return "airspace_e5";
        }
        if(local_type == "CLASS_E6")
        {
            return "airspace_e6";
        }
        if(local_type == "CLASS_E7")
        {
            return "airspace_e7";
        }
        return "airspace_e2";
    ...
    // Class airspace
    bool chart_style::airspace_visible(const std::string& airspace_class, const std::string& local_type,
                                       double zoom) const
    {
    ...
    {
        static constexpr std::array keys = {"airport_class_b", "airport_class_c", "airport_class_d", "airport_class_e",
                                            "airport_other"};
    ...
    {
        return visible("obstacle_1000ft", zoom) || visible("obstacle_200ft", zoom) || visible("obstacle_low", zoom);
    }

    bool chart_style::any_airway_visible(double zoom) const
    {
        static constexpr std::array keys = {"airway_v", "airway_t",  "airway_br",  "airway_tk", "airway_j",
                                            "airway_q", "airway_ar", "airway_rte", "airway_h",  "airway_m",
                                            "airway_l", "airway_y",  "airway_w",   "airway_n",  "airway_r",
                                            "airway_a", "airway_g",  "airway_b"};
        return std::any_of(keys.begin(), keys.end(), [&](const char* k) { return visible(k, zoom); });
    }

    bool chart_style::any_airspace_visible(double zoom) const
    {
        static constexpr std::array keys = {"airspace_b",  "airspace_c",  "airspace_d",  "airspace_e2", "airspace_e3",
                                            "airspace_e4", "airspace_e5", "airspace_e6", "airspace_e7"};
        return std::any_of(keys.begin(), keys.end(), [&](const char* k) { return visible(k, zoom); });
    }

    bool chart_style::any_sua_visible(double zoom) const
    {
        static constexpr std::array keys = {"sua_prohibited", "sua_restricted", "sua_warning",
    ...
    {
        return visible("artcc_low", zoom) || visible("artcc_high", zoom) || visible("artcc_unlimited_cta", zoom) ||
               visible("artcc_unlimited_fir", zoom) || visible("artcc_unlimited_uta", zoom);
    }
    ...
    chart_style::filter_list chart_style::visible_airport_classes(double zoom) const
    {
        ...
        if(visible("airport_other", zoom))
        {
            return std::nullopt;
        }
        std::vector<std::string> out;
        if(visible("airport_class_b", zoom))
        {
            out.emplace_back("B");
        }
        if(visible("airport_class_c", zoom))
        {
            out.emplace_back("C");
        }
        if(visible("airport_class_d", zoom))
        {
            out.emplace_back("D");
        }
        if(visible("airport_class_e", zoom))
        {
            out.emplace_back("E");
        }
        return out;
    ...
    chart_style::filter_list chart_style::visible_airspace_values(double zoom) const
    {
        ...
        std::vector<std::string> out;
        if(visible("airspace_b", zoom))
        {
            out.emplace_back("B");
        }
        if(visible("airspace_c", zoom))
        {
            out.emplace_back("C");
        }
        if(visible("airspace_d", zoom))
        {
            out.emplace_back("D");
        }
        if(visible("airspace_e2", zoom))
        {
            out.emplace_back("CLASS_E2");
        }
        if(visible("airspace_e3", zoom))
        {
            out.emplace_back("CLASS_E3");
        }
        if(visible("airspace_e4", zoom))
        {
            out.emplace_back("CLASS_E4");
        }
        if(visible("airspace_e5", zoom))
        {
            out.emplace_back("CLASS_E5");
        }
        if(visible("airspace_e6", zoom))
        {
            out.emplace_back("CLASS_E6");
        }
        if(visible("airspace_e7", zoom))
        {
            out.emplace_back("CLASS_E7");
        }
        return out;
    }
    ...
    chart_style::filter_list chart_style::visible_sua_types(double zoom) const
    {
        ...
        if(visible("sua_moa", zoom))
        {
            return std::nullopt;
        }
        std::vector<std::string> out;
        if(visible("sua_restricted", zoom))
        {
            out.emplace_back("RA");
        }
        if(visible("sua_prohibited", zoom))
        {
            out.emplace_back("PA");
        }
        if(visible("sua_warning", zoom))
        {
            out.emplace_back("WA");
        }
        if(visible("sua_alert", zoom))
        {
            out.emplace_back("AA");
        }
        if(visible("sua_nsa", zoom))
        {
            out.emplace_back("NSA");
        }
        return out;
    }
