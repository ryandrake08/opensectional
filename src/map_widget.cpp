...
namespace
{
    ...
    shader_bytecode get_msl_bytecode(shader_id id, sdl::shader_stage_t stage)
    {
        if(stage == sdl::shader_stage::vertex)
        {
            switch(id)
            {
            case shader_id::DEFAULT:  return { default_vert_metal, default_vert_metal_len };
            case shader_id::LINE:     return { line_vert_metal, line_vert_metal_len };
            case shader_id::TEXTURED: return { textured_vert_metal, textured_vert_metal_len };
            }
        }
        else
        {
            switch(id)
            {
            case shader_id::DEFAULT:  return { default_frag_metal, default_frag_metal_len };
            case shader_id::LINE:     return { line_frag_metal, line_frag_metal_len };
            case shader_id::TEXTURED: return { textured_frag_metal, textured_frag_metal_len };
            }
        }
        return { nullptr, 0 };
    }
...
struct map_widget::impl : public sdl::event_listener
{
    ...
    impl(sdl::device& dev, const char* tile_path,
        ...
        , tiles(tile_path
              ? std::make_unique<tile_renderer>(dev, tile_path)
              : nullptr)
    ...
    void rebuild_grid()
    {
        ...
        if(approx_zoom < 3)      grid_step = 30.0;
        else if(approx_zoom < 5) grid_step = 10.0;
        else if(approx_zoom < 7) grid_step = 5.0;
        else if(approx_zoom < 9) grid_step = 1.0;
    ...
    static double point_segment_distance_px(pixel_pos p,
                                             pixel_pos a,
                                             pixel_pos b)
    {
        auto dx = b.x - a.x;
    ...
    hit_route_segment(double click_lon, double click_lat) const
    {
        ...
        for(std::size_t i = 1; i < wps.size(); ++i)
        {
            auto arc = geodesic_interpolate(
                waypoint_lat(wps[i - 1]),
                waypoint_lon(wps[i - 1]),
                waypoint_lat(wps[i]),
                waypoint_lon(wps[i]));
    ...
    void open_info_popup(const feature& f,
                          double click_lon, double click_lat)
    {
        auto [alon, alat] = find_feature_type(feature_types, f)
...
void map_widget::focus_on_hit(const search_hit& hit)
{
    ...
    else
    {
        ...
        auto half_height_m =
            (lat_to_my(bbox->lat_max) -
             lat_to_my(bbox->lat_min)) * 0.5;
        auto half_width_m =
            (lon_to_mx(bbox->lon_max) -
             lon_to_mx(bbox->lon_min)) * 0.5;
...
const std::vector<std::unique_ptr<feature_type>>&
...
std::vector<search_hit> map_widget::search(
...
void map_widget::set_route(flight_route route)
{
    ...
    if(!wps.empty())
    {
        ...
        else
        {
            auto half_height_m =
                (lat_to_my(lat_max) -
                 lat_to_my(lat_min)) * 0.5;
            auto half_width_m =
                (lon_to_mx(lon_max) -
                 lon_to_mx(lon_min)) * 0.5;
            auto needed = std::max(half_height_m,
                                      half_width_m / d.view.aspect_ratio());
            d.view.half_extent_y = needed * 1.2;
            d.view.zoom(1.0);
