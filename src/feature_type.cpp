...
namespace mapbox
{
    namespace util
    {
        ...
        {
            static float get(const glm::vec2& p)
            {
                return p.x;
            }
        };
...
namespace osect
{
    namespace
    {
        ...
        void add_circle(polyline_data& pd, double cx, double cy, double r, const line_style& ls)
        {
            const auto n = 16;
        ...
        void add_circle_to(polyline_data& pd, double cx, double cy, double r, const line_style& ls, int n = 24)
        {
            std::vector<glm::vec2> pts;
            for(int i = 0; i < n; i++)
            {
        ...
        void add_seg_to(polyline_data& pd, double x0, double y0, double x1, double y1, const line_style& ls)
        {
            pd.polylines.push_back({glm::vec2(static_cast<float>(x0), static_cast<float>(y0)),
                                    glm::vec2(static_cast<float>(x1), static_cast<float>(y1))});
            pd.styles.push_back(ls);
        }

        void add_hexagon(polyline_data& pd, double cx, double cy, double r, const line_style& ls)
        {
            std::vector<glm::vec2> pts;
            for(int i = 0; i < 6; i++)
            {
        ...
        void add_rect(polyline_data& pd, double cx, double cy, double hw, double hh, const line_style& ls)
        {
            auto l = static_cast<float>(cx - hw);
            auto r = static_cast<float>(cx + hw);
            auto t = static_cast<float>(cy - hh);
            auto b = static_cast<float>(cy + hh);
            pd.polylines.push_back({{l, t}, {r, t}, {r, b}, {l, b}, {l, t}});
            pd.styles.push_back(ls);
        }

        void add_center_dot(polyline_data& pd, double cx, double cy, double r, const line_style& ls)
        {
            auto d = r * 0.15;
            ...
            pd.polylines.push_back({{left, fcy}, {fcx, up}, {right, fcy}, {fcx, down}, {left, fcy}});
            pd.styles.push_back(ls);
        }

        void add_caltrop(polyline_data& pd, double cx, double cy, double hex_r, const line_style& ls)
        {
        ...
        void add_comm_symbol(polyline_data& pd, double cx, double cy, double radius, const line_style& ls)
        {
                          ...
                          double mx_offset)
        {
            ...
            polyline.emplace_back(static_cast<float>(lon_to_mx(points[0].lon) + mx_offset),
                                  static_cast<float>(lat_to_my(points[0].lat)));
            for(std::size_t i = 1; i < points.size(); i++)
            {
                auto seg = geodesic_interpolate(points[i - 1].lat, points[i - 1].lon, points[i].lat, points[i].lon);
                for(std::size_t j = 1; j < seg.size(); j++)
                {
                    polyline.emplace_back(static_cast<float>(lon_to_mx(seg[j].lon) + mx_offset),
        ...
        void add_geodesic_circle(polyline_data& pd, double center_lat, double center_lon, double radius_nm,
                                 const line_style& ls, double mx_offset)
        {
        ...
        void add_letter(polyline_data& pd, const std::vector<float>& segs, double cx, double cy, double w, double h,
                        const line_style& ls)
        {
        ...
        bool sua_altitude_visible(const altitude_filter& af, int upper_ft, const std::string& upper_ref, int lower_ft,
...
#define FEATURE_TYPE_BODY(LBL, ID, TAG, FType)                                                                         \
    ...
    {                                                                                                                  \
        return std::holds_alternative<feature_t>(f);                                                                   \
    }                                                                                                                  \
    ...
    void build_selection(const build_context& ctx, const feature& f, polyline_data& out, polygon_fill_data& fill_out)  \
        const override;
...
#define DECLARE_POINT_FEATURE_TYPE(Class, LBL, ID, TAG, FType)                                                         \
    ...
    public:                                                                                                            \
        FEATURE_TYPE_BODY(LBL, ID, TAG, FType)                                                                         \
        std::optional<std::pair<double, double>> point_coord(const feature& f) const override                          \
        {                                                                                                              \
...
#undef FEATURE_TYPE_BODY
        ...
        void airport_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
        ...
        void comm_outlet_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.vis.altitude.low_enabled() || !ctx.styles.rco_visible(ctx.zoom))
            {
                return;
            }
        ...
        void awos_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.vis.altitude.low_enabled() || !ctx.styles.awos_visible(ctx.zoom))
            {
                return;
            }
        ...
        void airspace_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.vis.altitude.low_enabled())
            {
                return;
        ...
        void sua_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.vis.altitude.any())
            {
                return;
            ...
            {
                if(!ctx.styles.sua_visible(s.sua_type, ctx.zoom))
                {
                    continue;
                }
                for(const auto& stratum : s.strata)
                {
                    if(!ctx.vis.altitude.overlaps(stratum.lower_ft_val, stratum.lower_ft_ref, stratum.upper_ft_val,
        ...
        void artcc_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.vis.altitude.any())
            {
                return;
            ...
            {
                if(!ctx.styles.artcc_visible(a.altitude, a.type, ctx.zoom))
                {
                    continue;
                }
        ...
        void tfr_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.styles.tfr_visible(ctx.zoom))
            {
                return;
            }
            if(!ctx.vis.altitude.any())
            ...
            for(const auto& t : ctx.eph.tfrs().snapshot())
            {
                ...
                {
                    if(!ctx.vis.altitude.overlaps(area.lower_ft_val, area.lower_ft_ref, area.upper_ft_val,
                                                  area.upper_ft_ref))
                    {
                        continue;
                    }
        ...
        void adiz_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.styles.adiz_visible(ctx.zoom))
            {
                return;
            }
        ...
        void pja_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.vis.altitude.any())
            {
                return;
            ...
            if(!area_vis && !point_vis)
            {
                return;
            }
            for(const auto& p : ctx.db.query_pjas(ctx.pick_box))
            {
                auto upper = p.max_altitude_ft_msl > 0 ? p.max_altitude_ft_msl : altitude_filter::UNLIMITED_FT;
                if(!ctx.vis.altitude.overlaps(0, upper))
                {
                    continue;
                }
                auto is_point = (p.radius_nm <= 0);
                if(is_point ? !point_vis : !area_vis)
                {
                    continue;
                }
                auto hit = is_point ? (p.lon >= ctx.pick_box.lon_min && p.lon <= ctx.pick_box.lon_max &&
        ...
        void maa_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.vis.altitude.low_enabled())
            {
                return;
        ...
        void runway_type::pick(const pick_context& ctx, std::vector<feature>& out) const
        {
            if(!ctx.vis.altitude.low_enabled() || !ctx.styles.runway_visible(ctx.zoom))
            {
                return;
            }
        ...
        kv_list sua_type::info_kv(const feature& f) const
        {
            const auto& v = std::get<sua>(f);
            kv_list rows{
                {"Designator", v.designator}, {"Name", v.name},         {"Type", v.sua_type},
                {"Upper", v.upper_limit},     {"Lower", v.lower_limit},
            };
            ...
            auto fmt_endpoint = [](const std::string& clock, const std::string& event,
            ...
            {
                std::string val = (fr.tx == fr.rx || fr.rx.empty()) ? fr.tx : (fr.tx + "/" + fr.rx);
                if(!fr.mode.empty() && fr.mode != "OTHER")
                {
                    val += " " + fr.mode;
                }
                std::string flags;
                if(!fr.comm_allowed.empty())
                {
                    flags = fr.comm_allowed;
                }
        ...
        kv_list airway_type::info_kv(const feature& f) const
        {
            const auto& v = std::get<airway_segment>(f);
            return {
                {"ID", v.awy_id},         {"Location", v.awy_location}, {"From", v.from_point},
                {"To", v.to_point},       {"MEA", v.min_enroute_alt},   {"Mag crs/dist", v.mag_course_dist},
                {"Gap flag", v.gap_flag},
            };
        }
        ...
        void emit_airspace_label(const build_context& ctx, const std::string& type_text, double mx, double my,
                                 int layer, const std::string& upper, const std::string& lower, const feature_style& fs)
        {
        ...
        {
            return apt.ownership_type_code == "MA" || apt.ownership_type_code == "MR" ||
                               ...
                               const airport& apt, const feature_style& cs)
        {
            ...
            else if(apt.site_type_code == "H")
            {
                letter = &letter_H;
            }
            else if(apt.site_type_code == "C")
            {
                letter = &letter_S;
            }
            else if(apt.site_type_code == "U")
            {
                letter = &letter_F;
            }
            else if(apt.site_type_code == "G")
            {
                letter = &letter_G;
            }
            else if(apt.site_type_code == "B")
            {
                letter = &letter_B;
            }
            else if(mil)
        ...
        void add_triangle_polyline(polyline_data& pd, double cx, double cy, double r, const line_style& ls)
        {
        ...
        void add_waypoint_star_polyline(polyline_data& pd, double cx, double cy, double r, const line_style& ls)
        {
            ...
            for(const auto& [acx_off, acy_off, a0] : arcs)
            {
                for(int i = 0; i <= ARC_SEGS; i++)
                {
                    ...
                    star_pts.push_back({static_cast<float>(px), static_cast<float>(py)});
                }
            }
            ...
            for(int i = 0; i <= CIRCLE_SEGS; i++)
            {
                ...
                circle_pts.push_back(
                    {static_cast<float>(cx + cr * std::cos(angle)), static_cast<float>(cy + cr * std::sin(angle))});
            }
            pd.polylines.push_back(std::move(circle_pts));
            pd.styles.push_back(ls);
        }

        void emit_fix_icon(polyline_data& pd, double cx, double cy, double r, const fix& f, const feature_style& fs)
        {
            auto ls = to_line_style(fs);
        ...
        void add_obstacle_polylines(polyline_data& pd, double cx, double cy, double radius, bool tall, bool lighted,
                                    const line_style& ls)
        {
            auto dot_r = radius * 0.2;
            ...
            if(tall)
            {
                pd.polylines.push_back({
                    ...
                    glm::vec2(fcx, fapex_y),
                    glm::vec2(fcx, fmast_top),
                    glm::vec2(fcx, fapex_y),
        ...
        void emit_obstacle_icon(polyline_data& pd, double cx, double cy, double r, const obstacle& obs,
                                const feature_style& fs)
        {
            auto ls = to_line_style(fs);
        ...
        {
            if(!ctx.styles.any_airport_visible(ctx.req.zoom))
            {
                return;
            }
            const auto& airports =
            ...
            for(const auto& apt : airports)
            {
                ...
                ctx.labels.push_back({
                    ...
                    .priority = towered ? LABEL_PRIORITY_AIRPORT_TOWERED : LABEL_PRIORITY_AIRPORT_UNTOWERED,
        ...
        {
            if(!ctx.styles.runway_visible(ctx.req.zoom))
            {
                return;
            }
            ...
            const auto& runways = ctx.db.query_runways(request_bbox(ctx.req));
        ...
        {
            if(!ctx.styles.any_navaid_visible(ctx.req.zoom))
            {
                return;
            }
            ...
            for(const auto& nav : navaids)
            {
                ...
                ctx.state.navaid_positions.emplace_back(static_cast<float>(cx), static_cast<float>(cy));
                emit_navaid_icon(ctx.poly[layer_navaids], cx, cy, r, nav, fs);

                auto is_vor = nav.nav_type.find("VOR") != std::string::npos || nav.nav_type == "TACAN" ||
                              nav.nav_type == "VORTAC";
                ctx.labels.push_back({
                    .text = nav.nav_id,
                    .mx = lon_to_mx(nav.lon) + ctx.mx_offset,
                    .my = lat_to_my(nav.lat),
                    .priority = is_vor ? LABEL_PRIORITY_NAVAID_VOR : LABEL_PRIORITY_NAVAID_NDB,
                    .layer = layer_navaids,
                });
            }
        }
        ...
        void airway_type::build(const build_context& ctx) const
        {
            ...
            if(!ctx.styles.any_airway_visible(ctx.req.zoom))
            {
                return;
            }
            const auto& airways = ctx.db.query_airways(request_bbox(ctx.req));
            ...
            {
                if(!ctx.styles.airway_visible(seg.awy_id, ctx.req.zoom))
                {
                    continue;
                }
                ...
                auto arc = geodesic_interpolate(seg.from_lat, seg.from_lon, seg.to_lat, seg.to_lon);
                std::vector<glm::vec2> polyline;
            ...
            for(auto& [awy_id, segs] : label_groups)
            {
                std::sort(segs.begin(), segs.end(),
                          [](const seg_info& a, const seg_info& b) { return a.point_seq < b.point_seq; });
        ...
        {
            if(!ctx.styles.any_fix_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.any())
        ...
        {
            if(!ctx.styles.mtr_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.any())
        ...
        std::pair<double, double> view_center_mercator(const feature_build_request& req)
        {
        ...
        bool polygon_contains_mercator(const std::vector<airspace_point>& pts, double mx_offset, double qx, double qy)
        {
            ...
            for(size_t i = 0, j = pts.size() - 1; i < pts.size(); j = i++)
            {
                ...
                if((iy > qy) != (jy > qy) && qx < (jx - ix) * (qy - iy) / (jy - iy) + ix)
                {
                    inside = !inside;
                }
            }
        ...
        std::pair<double, double> circle_label_pos(double lat, double lon, double radius_nm, double mx_offset,
                                                   double view_mx, double view_my)
        {
        ...
        {
            if(!ctx.styles.any_sua_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.any())
            ...
            {
                if(!ctx.styles.sua_visible(seg.sua_type, ctx.req.zoom))
                {
                    continue;
                }
            ...
            {
                if(!sua_altitude_visible(ctx.req.altitude, c.upper_ft_val, c.upper_ft_ref, c.lower_ft_val,
                                         c.lower_ft_ref))
                {
                    continue;
                }
                auto ls = to_line_style(ctx.styles.sua_style(c.sua_type));
                add_geodesic_circle(ctx.poly[layer_sua], c.center_lat, c.center_lon, c.radius_nm, ls, ctx.mx_offset);
            }
            ...
            {
                if(!ctx.styles.sua_visible(s.sua_type, ctx.req.zoom))
                {
                    continue;
                }
                if(s.strata.empty())
                ...
                if(!sua_altitude_visible(ctx.req.altitude, st.upper_ft_val, st.upper_ft_ref, st.lower_ft_val,
                                         st.lower_ft_ref))
                {
                    continue;
                }
                if(st.parts.empty() || st.parts[0].points.empty())
                {
                    continue;
                }

                auto [vmx, vmy] = view_center_mercator(ctx.req);
                auto [cmx, cmy] = polygon_label_pos(st.parts[0].points, ctx.mx_offset, vmx, vmy);
                emit_airspace_label(ctx, sua_type_label(s.sua_type), cmx, cmy, layer_sua,
        ...
        void pja_type::build(const build_context& ctx) const
        {
            ...
            {
                auto upper = p.max_altitude_ft_msl > 0 ? p.max_altitude_ft_msl : altitude_filter::UNLIMITED_FT;
                if(!ctx.req.altitude.overlaps(0, upper))
                {
                    continue;
                }
                ...
                if(p.radius_nm > 0.0 && area_vis)
                {
                    ...
                    add_geodesic_circle(ctx.poly[layer_pja], p.lat, p.lon, p.radius_nm, ls, ctx.mx_offset);

                    if(ctx.req.zoom >= AIRSPACE_LABEL_MIN_ZOOM)
                    {
                        auto [vmx, vmy] = view_center_mercator(ctx.req);
                        auto [mx, my] = circle_label_pos(p.lat, p.lon, p.radius_nm, ctx.mx_offset, vmx, vmy);
                        std::string upper =
                            p.max_altitude_ft_msl > 0 ? format_altitude(p.max_altitude_ft_msl, "MSL") : "UNL";
                        emit_airspace_label(ctx, "PJA", mx, my, layer_pja, upper, "SFC", ctx.styles.pja_area_style());
                    }
                }
        ...
        void maa_type::build(const build_context& ctx) const
        {
            ...
            const auto& maas = ctx.db.query_maas(request_bbox(ctx.req));
            ...
            for(const auto& m : maas)
            {
                ...
                {
                    auto ls = to_line_style(ctx.styles.maa_area_style());
                    add_geodesic_circle(ctx.poly[layer_maa], m.lat, m.lon, m.radius_nm, ls, ctx.mx_offset);
                    is_area = true;
                }
                else if(point_vis)
                {
                ...
                if(is_area && ctx.req.zoom >= AIRSPACE_LABEL_MIN_ZOOM)
                {
                    ...
                    auto [mx, my] = !m.shape.empty()
                                        ...
                                        : circle_label_pos(m.lat, m.lon, m.radius_nm, ctx.mx_offset, vmx, vmy);
                    std::string upper = format_altitude(m.max_alt_ft, m.max_alt_ref.empty() ? "MSL" : m.max_alt_ref);
                    std::string lower = format_altitude(m.min_alt_ft, m.min_alt_ref.empty() ? "SFC" : m.min_alt_ref);
                    if(upper.empty())
                    {
                        upper = "UNL";
                    }
                    emit_airspace_label(ctx, "MAA", mx, my, layer_maa, upper, lower, ctx.styles.maa_area_style());
                }
            }
        }
        ...
        {
            if(!ctx.styles.tfr_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.any())
            ...
            for(const auto& t : tfrs)
            {
                for(const auto& area : t.areas)
                {
                    if(!sua_altitude_visible(ctx.req.altitude, area.upper_ft_val, area.upper_ft_ref, area.lower_ft_val,
                                             area.lower_ft_ref))
                    {
                        continue;
                    }
                    ...
                    if(!polyline_intersects_bbox(area.points, bbox))
                    {
                        continue;
                    }
                    auto [vmx, vmy] = view_center_mercator(ctx.req);
                    auto [cmx, cmy] = polygon_label_pos(area.points, ctx.mx_offset, vmx, vmy);
                    emit_airspace_label(ctx, "TFR", cmx, cmy, layer_tfr,
                                        format_altitude(area.upper_ft_val, area.upper_ft_ref),
                                        format_altitude(area.lower_ft_val, area.lower_ft_ref), fs);
                }
            }
        }
        ...
        {
            if(!ctx.styles.adiz_visible(ctx.req.zoom))
            {
                return;
            }
            auto bbox = request_bbox(ctx.req);
            ...
            {
                if(a.parts.empty() || a.parts[0].empty())
                {
                    continue;
                }
                auto [vmx, vmy] = view_center_mercator(ctx.req);
                auto [cmx, cmy] = polygon_label_pos(a.parts[0], ctx.mx_offset, vmx, vmy);
                emit_airspace_label(ctx, "ADIZ", cmx, cmy, layer_adiz, "UNL", "SFC", ctx.styles.adiz_style());
            }
        }

        void artcc_type::build(const build_context& ctx) const
        {
            if(!ctx.styles.any_artcc_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.any())
            ...
            {
                if(!ctx.styles.artcc_visible(a.altitude, a.type, ctx.req.zoom))
                {
                    continue;
                }
                ...
                auto [vmx, vmy] = view_center_mercator(ctx.req);
                auto [cmx, cmy] = polygon_label_pos(a.points, ctx.mx_offset, vmx, vmy);
                emit_airspace_label(ctx, "ARTCC", cmx, cmy, layer_artcc, a.location_id, a.altitude,
        ...
        {
            if(!ctx.styles.any_obstacle_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.low_enabled())
            ...
            {
                if(!ctx.styles.obstacle_visible(obs.agl_ht, ctx.req.zoom))
                {
                    continue;
                }
        ...
        {
            if(!ctx.styles.any_airspace_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.low_enabled())
            ...
            auto bbox = request_bbox(ctx.req);
            const auto& segs =
                ctx.db.query_class_airspace_segments(bbox, ctx.styles.visible_airspace_values(ctx.req.zoom));
            for(const auto& seg : segs)
            {
                if(!ctx.styles.airspace_visible(seg.airspace_class, seg.local_type, ctx.req.zoom))
                {
                    continue;
                }
                auto ls = to_line_style(ctx.styles.airspace_style(seg.airspace_class, seg.local_type));
            ...
            {
                if(!ctx.styles.airspace_visible(a.airspace_class, a.local_type, ctx.req.zoom))
                {
                    continue;
                }
                if(a.parts.empty() || a.parts[0].points.empty())
                {
                    continue;
                }
                auto [vmx, vmy] = view_center_mercator(ctx.req);
                auto [cmx, cmy] = polygon_label_pos(a.parts[0].points, ctx.mx_offset, vmx, vmy);
                const auto& fs = ctx.styles.airspace_style(a.airspace_class, a.local_type);
                // Undefined upper (class E) defaults to 18000 MSL (FL180)
                std::string upper =
                    a.upper_ref.empty() ? format_altitude(18000, "MSL") : format_altitude(a.upper_ft, a.upper_ref);
                std::string lower =
                    a.lower_ref == "SFC" || a.lower_ref.empty() ? "SFC" : format_altitude(a.lower_ft, a.lower_ref);
                emit_airspace_label(ctx, a.airspace_class, cmx, cmy, layer_airspace, upper, lower, fs);
            }
        }

        void comm_outlet_type::build(const build_context& ctx) const
        {
            if(!ctx.styles.rco_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.low_enabled())
        ...
        {
            if(!ctx.styles.awos_visible(ctx.req.zoom))
            {
                return;
            }
            if(!ctx.req.altitude.low_enabled())
        ...
        void ring_to_mercator(const std::vector<airspace_point>& pts, std::vector<glm::vec2>& out_ring)
        {
            ...
            out_ring.emplace_back(static_cast<float>(lon_to_mx(pts[0].lon)), static_cast<float>(lat_to_my(pts[0].lat)));
            for(std::size_t i = 1; i < pts.size(); i++)
            {
                auto seg = geodesic_interpolate(pts[i - 1].lat, pts[i - 1].lon, pts[i].lat, pts[i].lon);
                for(std::size_t j = 1; j < seg.size(); j++)
                {
                    out_ring.emplace_back(static_cast<float>(lon_to_mx(seg[j].lon)),
                                          static_cast<float>(lat_to_my(seg[j].lat)));
                }
            }
        }

        void circle_to_ring(double center_lon, double center_lat, double radius_nm, std::vector<glm::vec2>& out_ring)
        {
            auto pts = geodesic_circle(center_lat, center_lon, radius_nm);
            out_ring.clear();
            out_ring.reserve(pts.size());
            for(const auto& p : pts)
            {
                out_ring.emplace_back(static_cast<float>(lon_to_mx(p.lon)), static_cast<float>(lat_to_my(p.lat)));
            }
        }

        // Emit a white widened outline on top of an area feature. Same
        // line width as the normal outline plus 2×border + 2px padding.
        void emit_selection_boundary(polyline_data& out, const std::vector<glm::vec2>& ring, const line_style& base)
        {
            if(ring.size() < 2)
            {
                return;
        ...
        void fix_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                       polygon_fill_data& /*fill*/) const
        {
            const auto& v = std::get<fix>(f);
            auto g = point_selection_geom_for(v, ctx.req);
        ...
        void awos_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                        polygon_fill_data& /*fill*/) const
        {
            const auto& v = std::get<awos>(f);
            auto g = point_selection_geom_for(v, ctx.req);
        ...
        void comm_outlet_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                               polygon_fill_data& /*fill*/) const
        {
            const auto& v = std::get<comm_outlet>(f);
            auto g = point_selection_geom_for(v, ctx.req);
        ...
        void runway_type::build_selection(const build_context& /*ctx*/, const feature& /*f*/, polyline_data& /*out*/,
                                          polygon_fill_data& /*fill*/) const
        {
            // Runways have no selection overlay in the original code.
        }

        void pja_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                       polygon_fill_data& fill_out) const
        {
            const auto& v = std::get<pja>(f);
            if(v.radius_nm > 0.0)
            {
        ...
        void maa_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                       polygon_fill_data& fill_out) const
        {
            const auto& v = std::get<maa>(f);
        ...
        void airway_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                          polygon_fill_data& /*fill*/) const
        {
            const auto& v = std::get<airway_segment>(f);
            ...
            for(const auto& seg : ctx.db.query_airway_by_id(v.awy_id))
            {
                ...
                for(const auto& p : arc)
                {
                    polyline.emplace_back(static_cast<float>(lon_to_mx(p.lon)), static_cast<float>(lat_to_my(p.lat)));
                }
        ...
        void mtr_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                       polygon_fill_data& /*fill*/) const
        {
            const auto& v = std::get<mtr_segment>(f);
        ...
        void airspace_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                            polygon_fill_data& fill_out) const
        {
            const auto& v = std::get<class_airspace>(f);
                                       ...
                                       polygon_fill_data& fill_out) const
        {
            ...
            for(const auto& stratum : v.strata)
            {
                for(const auto& part : stratum.parts)
                {
                    if(part.is_circle)
                    {
                        circle_to_ring(part.circle_lon, part.circle_lat, part.circle_radius_nm, ring);
                    }
                    else
                    {
                        ring_to_mercator(part.points, ring);
        ...
        void artcc_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                         polygon_fill_data& fill_out) const
        {
            const auto& v = std::get<artcc>(f);
        ...
        void tfr_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                       polygon_fill_data& fill_out) const
        {
            const auto& v = std::get<tfr>(f);
        ...
        void adiz_type::build_selection(const build_context& ctx, const feature& f, polyline_data& out,
                                        polygon_fill_data& fill_out) const
        {
            const auto& v = std::get<adiz>(f);
        ...
        {
        public:
            std::pair<double, double> anchor_lonlat(const feature& f, double /*click_lon*/,
                                                    double /*click_lat*/) const override
            {
                const auto& v = std::get<pja>(f);
                return {v.lon, v.lat};
            }
        ...
        {
        public:
            std::pair<double, double> anchor_lonlat(const feature& f, double click_lon, double click_lat) const override
            {
                const auto& v = std::get<maa>(f);
                if(v.shape.empty())
                {
