...
namespace osect
{
    namespace
    {
        ...
        geo_bbox bbox_around(double lat, double lon, double radius_nm)
        {
            ...
            auto dlon = (cos_lat > 1e-6)
                        ? radius_nm / (NM_PER_DEG * cos_lat)
                        : 180.0;
            ...
            const std::string& code)
        {
            ...
            if(code == "G") return st::airport_gliderport;
            if(code == "H") return st::airport_heliport;
            if(code == "U") return st::airport_ultralight;
            ...
            const std::string& nav_type)
        {
            ...
            if(nav_type == "DME")     return st::navaid_dme;
            if(nav_type == "NDB")     return st::navaid_ndb;
            if(nav_type == "NDB/DME") return st::navaid_ndb_dme;
            ...
            const std::string& use_code)
        {
            ...
            if(use_code == "MR")  return st::fix_mr;
            if(use_code == "VFR") return st::fix_vfr;
        ...
        awy_class classify_airway_id(const std::string& id)
        {
            ...
            if(id.size() >= 2)
            {
                ...
                if(p2 == "AT" || p2 == "AR"
                   ...
                   || p2 == "PA" || p2 == "PR")
                    return ac::other;
                if(p2 == "TK") return ac::rnav;
            }
            switch(id.empty() ? '\0' : id[0])
            {
                case 'V': return ac::victor;
                case 'J': return ac::jet;
                case 'T':
                case 'Q': return ac::rnav;
                case 'G':
                case 'A':
                case 'R':
                case 'B': return ac::color;
                default:  return ac::other;
            }
        }
    ...
    struct route_planner::impl
    {
        ...
        std::size_t resolve_nearest(const std::string& id,
    ...
    route_planner::route_planner(const nasr_database& db)
        : pimpl(std::make_unique<impl>(db))
    {
        ...
        for(auto& e : db.load_airway_edges())
        {
            ...
            auto to_idx = pimpl->resolve_nearest(e.to_id,
                                                  e.to_lat, e.to_lon);
            if(from_idx == synthetic || to_idx == synthetic) continue;
            auto cls = classify_airway_id(e.awy_id);
            pimpl->neighbors[from_idx].push_back(
                {to_idx, e.awy_id, cls, e.is_gap});
            pimpl->neighbors[to_idx].push_back(
                {from_idx, std::move(e.awy_id), cls, e.is_gap});
        }
    }
    ...
    route_planner::node_index(const std::string& id) const
    {
        auto it = pimpl->index_by_id.find(id);
        if(it == pimpl->index_by_id.end()) return std::nullopt;
        return it->second;
    }

    const std::vector<route_planner::airway_edge>&
    ...
    namespace
    {
        ...
        // path reconstruction).
        constexpr auto PREV_FROM_ORIGIN =
    ...
    route_planner::plan_segment(const endpoint& origin,
                                 ...
                                 const options& opts) const
    {
        ...
        // is admissible.
        if(haversine_nm(origin.lat, origin.lon,
                        destination.lat, destination.lon) <= max_leg)
        ...
        for(std::size_t i = 0; i < opts.wp_cost.size(); ++i)
            min_wp = std::min(min_wp,
                effective_wp_cost(static_cast<wp_subtype>(i), opts));
        double min_awy = 1.0;
        if(opts.use_airways)
        {
        ...
        {
            return haversine_nm(nodes[n].lat, nodes[n].lon,
        ...
        // edges.
        auto edge_cost = [&](std::optional<wp_subtype> from_st,
                              wp_subtype to_st,
                              double dist_nm,
                              double airway_factor)
        {
            auto over_max = (dist_nm > max_leg) ? cost_reject : 1.0;
            auto from_mod = from_st
                ? effective_wp_cost(*from_st, opts)
                : 1.0;
        ...
        auto expand_around = [&](double from_lat, double from_lon,
            ...
            if(from_index != synthetic && from_index != PREV_FROM_ORIGIN)
            {
                for(const auto& e : airway_neighbors(from_index))
                {
                    ...
                    if(opts.use_airways)
                        awy_factor = e.is_gap
                            ? opts.gap_cost
                            : opts.awy_cost.at(static_cast<std::size_t>(e.type));
                    auto cost = edge_cost(from_st,
                                           nodes[e.neighbor_index].subtype,
                                           d, awy_factor);
        ...
        while(!open.empty())
        {
            ...
            if(haversine_nm(nodes[n].lat, nodes[n].lon,
                            destination.lat, destination.lon) <= max_leg)
            {
    ...
    namespace
    {
        ...
        project_walk_result project_and_walk(
            const std::string& airway_id,
            double lat, double lon,
            double toward_lat, double toward_lon,
            const nasr_database& db)
        {
            auto segs = db.query_airway_by_id(airway_id);
            if(segs.empty())
                throw route_parse_error(
                    "airway has no segments: " + airway_id);
            ...
            {
                auto d = point_to_segment_distance_nm(
                    segs[i].from_lat, segs[i].from_lon,
                    segs[i].to_lat, segs[i].to_lon, lat, lon);
                if(d < best_dist) { best_dist = d; best = i; }
            }

            const auto& s = segs[best];
            auto d_from = haversine_nm(
                s.from_lat, s.from_lon, toward_lat, toward_lon);
            auto d_to = haversine_nm(
                s.to_lat, s.to_lon, toward_lat, toward_lon);
            if(d_from <= d_to)
    ...
    std::string route_planner::expand_sigils(const std::string& text,
                                               const options& opts) const
    {
        ...
        for(std::size_t i = 0; i < tokens.size(); ++i)
            if(tokens[i] == "?") sigils.push_back(i);

        // Fast path: no sigils → unchanged.
        if(sigils.empty()) return text;
        ...
        for(auto s : sigils)
        {
            if(s == 0)
                throw route_parse_error(
                    "route cannot start with '?'", "?", static_cast<int>(s));
            if(s + 1 >= tokens.size())
                throw route_parse_error(
                    "route cannot end with '?'", "?", static_cast<int>(s));
            if(tokens[s - 1] == "?" || tokens[s + 1] == "?")
                throw route_parse_error(
                    "consecutive '?' not allowed", "?", static_cast<int>(s));
        }
        ...
        auto resolve_point = [&](const std::string& tok, int token_index)
            -> std::pair<double, double>
        {
            if(auto ll = parse_latlon(tok)) return {ll->lat, ll->lon};
            ...
            throw route_parse_error(
                "'" + tok + "' cannot be used as a route waypoint — "
        ...
        for(auto s : sigils)
        {
            ...
            for(auto k = cursor; k < s; ++k) out.push_back(tokens[k]);
            ...
            if(left_airway && right_airway)
                throw route_parse_error(
                    "'?' cannot have an airway on both sides",
                    "?", static_cast<int>(s));
            ...
            if(!left_airway && !right_airway)
            {
                ...
                auto path = plan_segment(
                    endpoint{synthetic, fl, gl},
                    endpoint{synthetic, fr, gr},
                    opts);
                if(!path)
                {
            ...
            else if(right_airway)
            {
                ...
                auto entry = project_and_walk(
                    right_tok, fl, gl, tl, gtl, pimpl->db);
                auto path = plan_segment(
                    endpoint{synthetic, fl, gl},
                    endpoint{synthetic, entry.lat, entry.lon},
                    opts);
                if(!path)
                {
                    std::string msg = "no route from ";
            ...
            else
            {
                ...
                auto [fr, gr] = resolve_point(right_tok, static_cast<int>(s + 1));
                auto [tl, gtl] = point_past_airway(s - 1, -1);
                auto exit = project_and_walk(
                    left_tok, fr, gr, tl, gtl, pimpl->db);
                out.push_back(exit.fix_id);
                auto path = plan_segment(
                    endpoint{synthetic, exit.lat, exit.lon},
                    endpoint{synthetic, fr, gr},
                    opts);
                if(!path)
                {
        ...
        for(auto k = cursor; k < tokens.size(); ++k) out.push_back(tokens[k]);
