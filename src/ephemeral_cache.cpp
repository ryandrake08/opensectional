...
namespace osect
{
    namespace
    {
        ...
        {
            if(name.empty()) throw std::runtime_error("empty source name");
            for(char c : name)
            {
                const bool ok =
                    (c >= 'a' && c <= 'z') ||
                ...
                if(!ok)
                    throw std::runtime_error(
                        "invalid source name (must be [A-Za-z0-9_-]): '" +
                        name + "'");
            }
        }

        std::filesystem::path file_for(
        ...
        std::string getenv_or_throw(const char* var)
        {
            ...
            if(!v || !*v)
                throw std::runtime_error(
                    std::string("environment variable not set: ") + var);
            return v;
        }
    ...
    std::filesystem::path ephemeral_cache::default_dir()
    {
        ...
        return std::filesystem::path(getenv_or_throw("HOME")) /
            "Library/Caches" / OSECT_BUNDLE_IDENTIFIER / "ephemeral";
#elif defined(_WIN32)
        return std::filesystem::path(getenv_or_throw("LOCALAPPDATA")) /
            "osect" / "ephemeral";
#else
        const char* xdg = std::getenv("XDG_CACHE_HOME");
        const auto base = (xdg && *xdg)
    ...
    ephemeral_cache::ephemeral_cache()
        : ephemeral_cache(default_dir())
    {
    }

    ephemeral_cache::ephemeral_cache(std::filesystem::path cache_dir)
        : dir_(std::move(cache_dir))
    {
        ...
        if(ec)
            throw std::runtime_error(
                "failed to create cache directory '" + dir_.string() +
                "': " + ec.message());
    }

    std::optional<ephemeral_cache::entry>
    ephemeral_cache::load(const std::string& source_name) const
    {
        ...
        if(!read_pod(in, magic) || magic != MAGIC) return std::nullopt;

        std::uint32_t version = 0;
        if(!read_pod(in, version) || version != VERSION) return std::nullopt;

        std::uint32_t etag_len = 0;
        if(!read_pod(in, etag_len)) return std::nullopt;
        // Sanity cap — ETags are short opaque strings, an 8 KiB
        // header would already be unreasonable.
        if(etag_len > 8 * 1024) return std::nullopt;
        ...
        if(!read_pod(in, body_len)) return std::nullopt;
        ...
        if(body_len > 256ULL * 1024 * 1024) return std::nullopt;
    ...
    void ephemeral_cache::store(const std::string& source_name,
                                 ...
                                 const std::string& etag)
    {
...

        {
            ...
            if(!out)
                throw std::runtime_error(
                    "cannot open cache temp file: " + tmp_path);
            ...
            if(!etag.empty())
                out.write(etag.data(),
                    static_cast<std::streamsize>(etag.size()));
            ...
            if(!out)
                throw std::runtime_error(
                    "write failed for cache temp file: " + tmp_path);
        ...
        if(ec)
        {
            ...
            throw std::runtime_error(
                "rename failed for cache file '" + final_path.string() +
                "': " + ec.message());
        }
    }
