...
namespace osect
{
    ...
    bool popup_manager::info_open_for_route() const
    {
        return pimpl->info.open && std::holds_alternative<route_pick>(pimpl->info.payload);
    }
    ...
    namespace
    {
                       ...
                       const std::vector<std::unique_ptr<feature_type>>& feature_types)
        {
            ...
            if(!p.items.empty())
            {
                ...
                // Group every pick by its feature_type's section tag.
                // route_pick is just one more variant of `feature`,
                // dispatched by route_type whose section_tag is
                // "ROUTE" — no special-casing needed here.
                std::vector<ui_section> sections(FEATURE_SECTION_COUNT);
                std::vector<std::vector<int>> section_item_index(FEATURE_SECTION_COUNT);
                ...
                for(int i = 0; i < static_cast<int>(p.items.size()); ++i)
                {
                    ...
                    section_item_index[s].push_back(i);
                }
                       ...
                       const std::optional<flight_route>& route_for_popup)
        {
            ...
            // get the generic "Flight route" header rather than the
            // route_type::summary (which describes the picked
            // waypoint/leg, not the whole route).
            std::string title;
            ...
            {
                title = find_feature_type(feature_types, p.payload).summary(p.payload);
            }
            ...
            // Body. route_picks render legs + Delete; everything
            // else renders the standard kv table.
            if(std::holds_alternative<route_pick>(p.payload))
            {
